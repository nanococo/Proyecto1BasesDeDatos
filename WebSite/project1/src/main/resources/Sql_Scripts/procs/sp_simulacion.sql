        ...
        WHILE @fechaInicio <= @fechaFin
            BEGIN
                ...
                IF EXISTS(SELECT [Fecha]
                          FROM OPENXML(@hdoc, '' Operaciones / FechaOperacion '', 1) WITH ([Fecha] DATE)
                          WHERE [Fecha] = @fechaInicio)
                    BEGIN

                        --INSERT DE PERSONAS
                        INSERT INTO [dbo].[Persona] ([ValorDocumentoIdentidadDelCliente],
                                                     [TipoDocIdentidadId],
                                                     [Nombre],
                                                     [FechaNacimiento],
                                                     [Email],
                        ...
                        FROM OPENXML(@hdoc, '' Operaciones / FechaOperacion / Persona '', 1)
                                     WITH ([Fecha] DATE ''../@Fecha'', [ValorDocumentoIdentidad] VARCHAR(20),
                        ...
                        --INSERT DE CUENTAS
                        INSERT
                        INTO [dbo].[CuentaAhorros] ([NumeroCuenta],
                                                    [PersonaId],
                                                    [TipoCuentaId],
                                                    [FechaCreacion])
                        SELECT [NumeroCuenta],
                        ...
                        FROM OPENXML(@hdoc, '' Operaciones / FechaOperacion / Cuenta '', 1)
                                     WITH (
                                         [Fecha] DATE ''../@Fecha'', [NumeroCuenta] INT,
                        ...
                        INSERT
                        INTO [dbo].[Beneficiarios] ([PersonaId],
                                                    [CuentaAsociadaId],
                                                    [Porcentaje],
                                                    [ParentescoId])
                        SELECT [P].[Id],
                        ...
                        FROM OPENXML(@hdoc, '' Operaciones / FechaOperacion / Beneficiario '', 1)
                                     WITH ([Fecha] DATE ''../@Fecha'', [NumeroCuenta] INT,
                                 ...
                                 INNER JOIN dbo.Persona AS P
                                            ON D.ValorDocumentoIdentidadBeneficiario =
                                               P.ValorDocumentoIdentidadDelCliente
                        WHERE [Fecha] = @fechaInicio


                        --INSERT DE MOVIMIENTOS
                        INSERT
                        INTO [dbo].[Movimientos] ([CuentaAhorrosId],
                                                  [TipoMovId],
                                                  [Fecha],
                                                  [Monto],
                                                  [Descripcion])
                        ...
                        FROM OPENXML(@hdoc, '' Operaciones / FechaOperacion / Movimientos '', 1)
                                     WITH ([Fecha] DATE ''../@Fecha'', [Tipo] INT,
                                         [CodigoCuenta] INT,
                                         [Monto] MONEY,
                        ...
                        --INSERT DE USUARIOS
                        INSERT INTO [dbo].[Usuarios] ([PersonaId],
                                                      [Username],
                                                      [Password],
                                                      [EsAdmin])
                        ...
                        FROM OPENXML(@hdoc, '' Operaciones / FechaOperacion / Usuario '', 1)
                                     WITH (
                                         [Fecha] DATE ''../@Fecha'', [ValorDocumentoIdentidad] INT,
                                         [User] VARCHAR(20),
                                         [Pass] VARCHAR(50),
